- [x] cover from bundle name instead of shard name

- [x] move conditional to filter
- [x] check # in 21b,

- [ ] add create render context function, put templates/fields/css/<filename,nvid> map in them, elimanite query bundle things each render() call
- [ ] move audio to filter, remove cook feature. can use nvid map in ctx

- [ ] move _removeTemplate/_replaceAsync/parseFields/_processMediaFile to template, name proper names


- [ ] each some filter, leave low priority ones empty
- [ ] filter cat into several files, prioritiezed

- [ ] cloze && Add cloze conditional detection (/^c\d+$/)