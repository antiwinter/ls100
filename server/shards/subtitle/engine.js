...
export const engine = {
  ...
  async processCreate(shard, data) {
    ...
    return await this._processSubtitles(shard, data, true)
  },
  ...
  async processUpdate(shard, data, _updateData) {
    ...
    return await this._processSubtitles(shard, data, false)
  },
  
  // Internal: Handle subtitle linking logic (shared between create/update)
  async _processSubtitles(shard, data, isCreate) {
    ...
    if (isCreate && data.initialContent?.subtitle_id) {
      ...
      await subtitleData.linkSubtitle(shard.id, subtitleId, true)
    }
      ...
      if (isCreate) {
        ...
        for (const language of data.languages) {
          if (language.subtitle_id) {
            await subtitleData.linkSubtitle(shard.id, language.subtitle_id, language.isMain || false)
          }
        }
      } else {
        // Update: Compare current vs desired and sync
        const currentSubtitles = await subtitleData.getSubtitles(shard.id)
        ...
        for (const subtitleId of toUnlink) {
          await subtitleData.unlinkSubtitle(shard.id, subtitleId)
        }
  ...
  async getData(shardId) {
    const subtitles = await subtitleData.getSubtitles(shardId)
    return {
